...
export default function PostCard({
  ...
  return (
    <Link
      href={href}
      className={`toss-card group block overflow-hidden rounded-card bg-white shadow-card hover:shadow-card-hover ${
        isFeatured ? "h-full" : ""
      }`}
    >
      {post.cover ? (
        ...
        >
          ...
          <img
            src={post.cover}
            alt={post.title}
            className="h-full w-full object-cover transition-transform duration-700 ease-toss group-hover:scale-[1.04]"
            loading="lazy"
          />
        </div>
      ) : (
